...
const char* mqtt_server = "10.207.231.164";  // Rqspberry IP
const int mqtt_port = 1883;
const char* mqtt_topic = "gesture/control";
...
extern byte WIFI_MODE = 2;  // select WIFI_MODE in app_httpd.cpp
...
// Control functions mapped to existing behavior
void moveForward() {
  funcMode = 0;
  moveFB = 1;
  moveLR = 0;
...
void stopMovement() {
  funcMode = 0;  // Stay in "can be controlled" mode
  moveFB = 0;
  moveLR = 0;
}
void actionStayLow() {
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  else if (command == "STEADY") setSteady();
  else if (command == "HANDSHAKE") actionHandshake();
  else if (command == "JUMP") actionJump();
  else if (command == "STAYLOW") actionStayLow();
}
      ...
      if (docReceive["var"] == "funcMode") {
        ...
        switch (val) {
          case 1:
            ...
            if (gestureUD > gestureOffSetMax) { gestureUD = gestureOffSetMax; }
            break;
          case 2:
            gestureUD -= gestureSpeed;
            if (gestureUD < -gestureOffSetMax) { gestureUD = -gestureOffSetMax; }
          ...
          case 3: break;
          case 4:
            gestureLR -= gestureSpeed;
            if (gestureLR < -gestureOffSetMax) { gestureLR = -gestureOffSetMax; }
            break;
          case 5:
            gestureLR += gestureSpeed;
            if (gestureLR > gestureOffSetMax) { gestureLR = gestureOffSetMax; }
        ...
        switch (val) {
          ...
          case 1:
            ...
            setSingleLED(1, matrix.Color(0, 32, 255));
            break;
          case 2:
            setSingleLED(0, matrix.Color(255, 32, 0));
            setSingleLED(1, matrix.Color(255, 32, 0));
            break;
          case 3:
            setSingleLED(0, matrix.Color(32, 255, 0));
            setSingleLED(1, matrix.Color(32, 255, 0));
            break;
          case 4:
            setSingleLED(0, matrix.Color(255, 255, 0));
            setSingleLED(1, matrix.Color(255, 255, 0));
            break;
          case 5:
            setSingleLED(0, matrix.Color(0, 255, 255));
            setSingleLED(1, matrix.Color(0, 255, 255));
            break;
          case 6:
            setSingleLED(0, matrix.Color(255, 0, 255));
            setSingleLED(1, matrix.Color(255, 0, 255));
            break;
          case 7:
            setSingleLED(0, matrix.Color(255, 64, 32));
            setSingleLED(1, matrix.Color(32, 64, 255));
            break;
        }
      }
...
void setup() {
  ...
  Serial.begin(115200);
  delay(1000);  // Give time for USB to initialize
